...
/**
 * The monitoring data of response times. Every time a internal action is
 * executed, a {@link ResponseTimeRecord} record is stored.
 *
 * @author JP
 *
 */
public interface ResponseTimeDataSet {
	...
	List<ResponseTimeRecord> getResponseTimes(String parent);

	/**
	 * Gets the IDs of all monitored resource types for an internal action.
	 * 
	 * @param internalActionId The id of the internal action.
	 * 
	...
	/**
	 * Gets all records for a specific internal action ID and resource type ID. The
	 * internal action ID is the value returned by {@link InternalAction#getId()}
	 * and the resource type ID the value returned by {@link ResourceType#getId()}.
	 * 
	 * @param internalActionId The ID of the internal action.
	 ...
	 *
	 * @param time The input for the conversion.
