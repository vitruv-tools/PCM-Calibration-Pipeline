...
public class LibredeResourceDemandEstimation {
	 ...
	 * 
	 * @param parametricDependencyEstimationStrategy The strategy for estimating
	 *                                               resource demand model.
	 * @param resourceUtilization                    The resource utilization of the
	 *                                               monitored internal actions.
	 * @param responseTimes                          The response time records.
	 * @param serviceCalls                           The service call records.
	 */
	...
	/**
	 * Gets for each resource demand in the {@link ResponseTimeDataSet} a resource
	 * demand model.
	 * 
	 * @return A map, which maps internal action IDs to a map, which maps resource
	 *         type IDs to their corresponding resource demand model.
	 */
	...
	/**
	 * Gets for each resource demand in the {@link ResponseTimeDataSet} the
	 * different service call parameters with the average response time.
	 * 
	 * @return A map, which maps internal action IDs to a map, which maps resource
	 *         type IDs to a map, which map service parameters to the average
	 *         resource demand.
	 */
	 ...
	 * 
	 * @param filePath The configuration file. An existing file is replaced.
	 */
	...
	private void addResourceUtilization(final String resourceId, final SortedMap<Long, Double> values,
		...
		configuration.setInterval(UnitsFactory.eINSTANCE.createQuantity(5, Time.SECONDS));
	...
	private void buildLibredeConfig() {
		...
		Quantity<Time> endTime = unitsFactory.createQuantity(
				this.responseTimeRepository.timeToSeconds(this.responseTimeRepository.getLatestEntry()), Time.SECONDS);

		startTime = startTime.minus(unitsFactory.createQuantity(5, Time.SECONDS));
		endTime = endTime.plus(unitsFactory.createQuantity(5, Time.SECONDS));
		...
		Quantity<Time> halfMinute = unitsFactory.createQuantity(1, Time.SECONDS);
